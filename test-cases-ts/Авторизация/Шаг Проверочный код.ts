...
export const otpStep: TestCase[] = [
    ...
    steps: [
      {
        ...
        expected: ['Legal-текст отображается', 'Legal-текст содержит ссылку на "/api/otps"']
      }
    ]
  },
  {
  ...
  },
  {
    ...
    steps: [
      {
        action:
          'Ввести отп код для введенного телефона из источника /api/otps и нажать кнопку "Войти"',
        expected: [
  ...
  },
  {
    ...
    steps: [
      {
        action:
          'Авторизоваться по ссылке с параметром redirect, равным какой-то из страниц системы,например %2Fhistory',
